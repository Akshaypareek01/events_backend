...
function createSesTransport() {
  ...
  const secure =
    process.env.SES_SMTP_SECURE === "true" || port === 465;
...
function createTransport() {
  if (sesConfigured()) return createSesTransport();
...
export async function sendMailSafe(options: {
  ...
  if (!isEmailConfigured()) {
    console.warn(
      `[email] Mail not configured — would send to ${options.to}: ${options.subject}`,
    );
    return;
  }
