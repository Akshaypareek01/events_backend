...
function mapCompany(doc: { _id: unknown; name: string; couponCode: string; createdAt?: Date; updatedAt?: Date }) {
  return {
    id: String(doc._id),
    name: doc.name,
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const p = parsed.data;
    if (p.name !== undefined) doc.name = p.name.trim();
    if (p.couponCode !== undefined) doc.couponCode = normalizeCouponCode(p.couponCode);
    try {
      await doc.save();
    } catch (e: unknown) {
      if (
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (linked > 0) {
      throw new ApiError(
        409,
        `Cannot delete: ${linked} user(s) are registered under this company`,
        { code: "COMPANY_IN_USE" },
      );
    }
