...
adminClassesRouter.get("/classes", asyncHandler(async (_req, res) => {
    const list = await ClassSession.find().sort({ type: 1, createdAt: 1 });
    res.json({ classes: list.map(mapClass) });
}));
adminClassesRouter.post("/classes", asyncHandler(async (req, res) => {
...
adminClassesRouter.patch("/classes/:id", asyncHandler(async (req, res) => {
...
adminClassesRouter.delete("/classes/:id", asyncHandler(async (req, res) => {
    const id = String(req.params.id);
    if (!mongoose.Types.ObjectId.isValid(id)) {
        throw new ApiError(400, "Invalid class id", { code: "VALIDATION" });
    }
